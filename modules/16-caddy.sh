...
build_and_install_alidns_caddy() {
    local xcaddy_bin build_dir current_diversion
    ...
    if ! XCADDY_SUDO=0 "$xcaddy_bin" build --output "$build_dir/caddy" --with "$CADDY_PLUGIN"; then
        rm -rf "$build_dir"
        log_error "自定义 Caddy 构建失败"
        return 1
    fi
    if ! caddy_binary_has_alidns "$build_dir/caddy"; then
        rm -rf "$build_dir"
    ...
    sudo install -m 755 -o root -g root "$build_dir/caddy" /usr/bin/caddy.custom
...
if [ -z "$caddy_bin" ] || ! caddy_binary_has_alidns "$caddy_bin"; then
    validate_caddy_build_prerequisites
fi
