...
# A profile generated by an older setup release must gain GLFW support without
# duplicating the existing Fcitx variables.
legacy_profile="$fixture_root/legacy-profile"
cat > "$legacy_profile" <<'EOF'
# BEGIN homelab Fcitx5 environment
export GTK_IM_MODULE=fcitx
export QT_IM_MODULE=fcitx
export XMODIFIERS=@im=fcitx
# END homelab Fcitx5 environment
EOF
ensure_fcitx_profile "$legacy_profile"
[ "$(grep -c '^export GTK_IM_MODULE=fcitx$' "$legacy_profile")" -eq 1 ] || fail "GTK input environment was duplicated"
[ "$(grep -c '^export QT_IM_MODULE=fcitx$' "$legacy_profile")" -eq 1 ] || fail "Qt input environment was duplicated"
[ "$(grep -c '^export XMODIFIERS=@im=fcitx$' "$legacy_profile")" -eq 1 ] || fail "XMODIFIERS was duplicated"
[ "$(grep -c '^export GLFW_IM_MODULE=ibus$' "$legacy_profile")" -eq 1 ] || fail "GLFW input environment was not added"
legacy_first="$(cat "$legacy_profile")"
ensure_fcitx_profile "$legacy_profile"
[ "$(cat "$legacy_profile")" = "$legacy_first" ] || fail "upgraded Fcitx profile was not idempotent"
...
printf '%s|%s|%s|%s|%s|%s\n' \
    ...
    "$GTK_IM_MODULE" "$QT_IM_MODULE" "$XMODIFIERS" "$GLFW_IM_MODULE"
EOF
...
[ "$startwm_output" = 'unix:path=/tmp/test-private-bus||fcitx|fcitx|@im=fcitx|ibus' ] || fail "startwm did not isolate D-Bus and preserve the input environment"
